...
```{r fig.width=16, fig.height=16, fig_ggplot2_pairs_1}
require(ggplot2) 
require(dplyr)
require(tidyr)
...
```{r fig.width=16, fig.height=16, fig_ggplot2_pairs_2}
wine_df1 %>% 
...
```{r fig.width=12, fig.height=12, fig_featurePlot_1}
...
```{r fig.width=12, fig.height=12, fig_featurePlot_2}
featurePlot(x = wine_df1[, 1:11], 
...
We represent the data projected in the plane of the two principal components. The direction of the features are as well represented in the same plane. Two elipses are showing the 0.68 probability boundary for the distribution of the six groups of **quality** (3-8). A circle superposed over the scatter plot data helps to evaluate the relative ratio between the features in the most important principal components plane. The features with highest dimmensions or aligned with the leading principal component are the ones with highest variance.


```{r fig.width=12, fig.height=12, fig_pca_transform_biplot}
q.diag = wine_df1[,12]
...
rtab <- table(testset$quality,testset$predicted)
