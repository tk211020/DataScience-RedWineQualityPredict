...
evulate <- function(evulate_type, file, df, k){
  ...
  max <- 0
  for(i in c(1:(k-1))){
    tmp <- cv(evulate_type, file, datas[((i - 1) * each + 1):(i * each),],
  ...
  tmp <- cv(evulate_type, file, datas[((k - 1) * each + 1):nr, ],
            datas[1:each, ],
...
handle_data <- function(args){
  ...
  } else if (!exists("model_file")){
    stop("missing argument --model ", call.=FALSE)
  ...
  # cross validation
  tmp <- evulate(evulate_type, model_file, train_df, strtoi(fold))
...
if (length(args) == 0){
  # local test
  system("Rscript final_project.R --fold 5 --train winequality-red.csv --report CART_performance_recall.csv --model CART.rds --evulate recall")
} else {
  handle_data(args)
}